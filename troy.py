...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if 'processing' in context.user_data and context.user_data['processing']:
        await update.message.reply_text("⚠️ A genration process is already running. Please wait until it completes or cancel it.")
        return
...
async def purchase(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("🛒 Purchase", url="")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    try:
        ...
        conv_handler = ConversationHandler(
            entry_points=[
                ...
                MessageHandler(filters.Text(['🔐 Start Generation']), crypt)  # Handle menu button
            ],
